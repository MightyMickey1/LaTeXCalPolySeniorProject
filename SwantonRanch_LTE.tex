...
\begin{figure}[!ht]
    \centering
    \includegraphics[max width=\linewidth]{Images/what-is-CBRS-architecture.png}
    ...
    (Tier~2), and General Authorized Access (Tier~3) \autocite{gieske2024cbrs}.}
...
\subsection{RF Propagation Simulation Tool Accessibility}
...
predecessors. Previously, ray-tracing RF simulation capable of the fidelity
eino.ai provides required a dedicated workstation running specialized thick-client
software that was prohibitively expensive. That kind of tool was available
only to large telecom engineering firms and carriers. Today, eino.ai delivers
equivalent capability through a web browser at a much more accessible cost. The shift
...
hardware and spectrum access described in Section~I.A. All three
...
\begin{figure}[!ht]
    \centering
    % scale=0.7 + transform shape scales coordinates AND text uniformly.
    % Coordinates are in pre-scale cm; rendered width = 20.8 cm x 0.7 = 14.56 cm.
    \begin{tikzpicture}[x=1cm, y=1cm, scale=0.7, transform shape, >=Stealth,
        box/.style={draw, rectangle, rounded corners=3pt, align=center,
                    inner sep=5pt, minimum width=2.5cm, minimum height=0.9cm},
        grp/.style={draw, dashed, thick, rectangle, rounded corners=6pt,
                    inner sep=9pt},
        arr/.style={->, semithick},
        biarr/.style={<->, semithick},
        rf/.style={->, semithick, dashed},
        lbl/.style={fill=white, inner sep=2pt, text centered}
    ]
    % --- User Equipment ---
    \node[box] (ue1) at (0, 2.2) {Android UE 1\\PHONE-10};
    \node[box] (ue2) at (0, 0.0) {Android UE 2\\PHONE-20};
    \node[box] (ue3) at (0, -2.2) {Android UE 3\\PHONE-30};
    % --- eNodeB ---
    \node[box, minimum height=6.2cm, minimum width=3.0cm] (enb) at (5.0, 0)
        {Baicells\\Nova~846\\eNodeB\\[4pt]
         \rule{2.2cm}{0.3pt}\\[3pt]
         AW3376-E-F\\Sector Antenna\\
         4T4R $\cdot$ Band~48};
    % --- LAN: Private Router ---
    \node[box, minimum width=3.0cm] (router) at (11.0, 1.5)
        {Private Router\\10.0.1.0/24};
    % --- LAN: Intel NUC / Open5GS ---
    \node[box, minimum width=3.0cm] (nuc) at (11.0, -1.5)
        {Intel NUC\\\textit{Open5GS}\\10.0.1.4\\MME $\cdot$ HSS $\cdot$ PGW};
    % --- LAN grouping ---
    \begin{scope}[on background layer]
        \node[grp, fit=(router)(nuc)] (lan) {};
    \end{scope}
    \node[anchor=south] at ([yshift=5pt]lan.north) {LAN: 10.0.1.0/24};
    % --- Starlink ---
    \node[box, minimum width=2.5cm] (sl) at (15.5, 1.5) {Starlink\\Terminal};
    % --- Internet ---
    \node[box, minimum width=2.5cm] (inet) at (18.3, 1.5) {Internet};
    % --- RF arrows (dashed = wireless) ---
    \draw[rf] (ue1.east) -- (enb.west |- ue1.east);
    \draw[rf] (ue2.east) -- node[lbl, above] {LTE RF} (enb.west);
    \draw[rf] (ue3.east) -- (enb.west |- ue3.east);
    % --- eNodeB to Router (S1 / Ethernet) ---
    \draw[arr] (enb.east |- router.west) -- (router.west)
        node[inner sep=2pt, above, pos=0.5] {S1 / Ethernet};
    % --- eNodeB to NUC (S1AP control plane) ---
    \draw[arr] (enb.east |- nuc.west) -- (nuc.west)
        node[inner sep=2pt, below, pos=0.5] {S1AP/SCTP};
    % --- Router to NUC (LAN) ---
    \draw[biarr] (router.south) -- (nuc.north);
    % --- Router to Starlink (WAN backhaul) ---
    \draw[arr] (router.east) -- node[lbl, above] {WAN} (sl.west);
    % --- Starlink to Internet ---
    \draw[arr] (sl.east) -- (inet.west);
    \end{tikzpicture}
    ...
    backhaul layers. Dashed arrows indicate the wireless LTE air interface
    (Band~48, 3.655~GHz, PLMN 315-010); solid arrows indicate wired
...
\textbf{Data processing:} A Python script filtered the raw logs to retain only
rows where the serving operator was
...
\begin{figure}[!ht]
    \centering
    \rotatebox{-90}{\includegraphics[height=\linewidth]{Images/IMG_3314.jpeg}}
...
Shortly after initial RF enable, the radio began
...
a Python script. The script retained only
...
north-south and 2~km east-west.
...
Three lessons from the development and deployment process are worth documenting
...
\textbf{2. GPS lock takes time; plan for it.}
...
\textbf{3. Verify G-NetTrack is actively logging before starting a drive test
route.}
...
\begin{table}[!ht]
    ...
    \begin{tabular}{p{0.22\linewidth}p{0.30\linewidth}p{0.16\linewidth}p{0.14\linewidth}}
        ...
        Baicells Nova 430i & Indoor eNodeB, Band 48, lab validation
            & Borrowed from 5G Lab & \$1,749.00 \\
        \addlinespace
        Intel NUC 7 i5BNK & Small form factor PC, Ubuntu Server 22.04 host
            & Borrowed & {\$489.99} \\
        \addlinespace
        ...
        Programmable SIM cards (3$\times$) & Blank LTE SIM cards
            & Purchased & \$12.99 \\
        \addlinespace
        Private LAN router (Apple AirPort Extreme) & Small router for 10.0.1.0/24 LAN
            & Borrowed & $\sim$\$35.00 \\
        \addlinespace
        ...
        \midrule
        \textbf{Total estimated MSRP} & & & \textbf{\$6,184.01} \\
        \addlinespace
        \textbf{Total student cost} & & & \textbf{\$91.96} \\
        \bottomrule
    \end{tabular}
    \label{tab:parts}
\end{table}
...
[PLACEHOLDER TEXT SINCE THIS APPENDIX WILL BE REMOVED]

\begin{table}[!ht]
    \centering
    \caption[Summary Statistics]{Summary statistics for data points captured.}
    \vspace{0.5\baselineskip}
    \begin{tabular}{lllll}
        \toprule
